...
import time
from repscat.aux_funcs import interpolate_tmat


def get_k1_list(xi, k2, len_k1):
    k1_list = np.linspace(
        k2 * np.exp(-2*np.abs(xi)), k2 * np.exp(2*np.abs(xi)), len_k1
    )
    # k_list = np.linspace(k2*np.exp(-np.abs(xi)), k2*np.exp(np.abs(xi)), len_k1)
    # k1_list = np.linspace(
...
def boost_tmat_at_k2_vals(xi, k_list, tmat_diag_vals, jay_max, k2, len_k1):
    ...
    for i_k1, k1 in np.ndenumerate(k1_list):
        ...
        tmat_boosted_at_k2[i_k1] = np.einsum(
